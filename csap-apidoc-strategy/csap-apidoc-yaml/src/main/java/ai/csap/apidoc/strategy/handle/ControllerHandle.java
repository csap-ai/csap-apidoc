 ...
 */
@AllArgsConstructor
public class ControllerHandle extends AbstractMapHandle<StandardProperties, String, CsapDocModelController> {
    /**
     * 文档配置
     */
    @Getter
    ...
    @Override
    public Map<String, CsapDocModelController> handle(ApidocStrategyName strategyType, String path, String prefixName) {
        return convertYaml(new TypeReference<>() {
        }, strategyType, path, prefixName).map(this::flatMap).orElseGet(Maps::newHashMap);
    }

    @Override
    public Handle<StandardProperties, Map<String, CsapDocModelController>> handle(StandardProperties standardProperties, ApidocStrategyName strategyType, String path) {
        standardProperties.setControllerMap(handle(strategyType, path));
        return this;
    }
}
